...
// Submission verification queue. §13.10 / §16. admin + super_admin.
export default async function AdminQueuePage() {
  ...
  const { data: pending } = await supabase
    .from("submissions")
    // Disambiguate the embed: submissions has two FKs to profiles
    // (participant_id and reviewed_by). Without the explicit FK, PostgREST
    // returns PGRST201 and no rows.
    .select(
      "id, participant_id, challenge_day, hours_claimed, ocr_extracted_hours, topic, whatsapp_post_time, submitted_at, flag_reasons, internal_notes, screenshot_path, profiles!submissions_participant_id_fkey(full_name, course, hall_id)"
    )
